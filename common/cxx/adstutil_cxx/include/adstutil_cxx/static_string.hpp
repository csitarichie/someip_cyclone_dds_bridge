...
#        define AK_TOOLKIT_STRING_VIEW_OPERATIONS()                       \
            constexpr operator ::std::experimental::string_view() const   \
            {                                                             \
                return ::std::experimental::string_view(c_str(), size()); \
            }
#    elif AK_TOOLKIT_CONFIG_USING_STRING_VIEW == 3
#        include <boost/utility/string_ref.hpp>
#        define AK_TOOLKIT_STRING_VIEW_OPERATIONS()          \
            constexpr operator ::boost::string_ref() const   \
            {                                                \
...
public:
    constexpr string(const char (&lit)[N + 1])
        : _lit((AK_TOOLKIT_ASSERT(lit[N] == 0), lit))
    {
    }
    constexpr char operator[](int i) const
    {
        return AK_TOOLKIT_ASSERT(i >= 0 && i < N), _lit[i];
    }
...
class string<N, char_array>
{
    ...
    constexpr explicit string(private_ctor, string<M, TL> const& l, string<N - M, TR> const& r,
                              detail::int_sequence<Il...>, detail::int_sequence<Ir...>)
        : _array{l[Il]..., r[Ir]..., 0}
    {
    }

    template <int... Il, typename T>
    constexpr explicit string(private_ctor, T const& l, detail::int_sequence<Il...>, int offset)
...
public:
    ...
    constexpr char operator[](int i) const
    {
        return AK_TOOLKIT_ASSERT(i >= 0 && i < N), _array[i];
    }
};
...
class string<N, array_suffix>
{
    ...
    template <int M, int... Il, int... Ir, typename TL, typename TR>
    constexpr explicit string(private_ctor, string<M, TL> const& l, string<N - M, TR> const& r,
                              detail::int_sequence<Il...>, detail::int_sequence<Ir...>, int lsize, int rsize)
...
public:
    ...
    constexpr char operator[](int i) const
    {
        return AK_TOOLKIT_ASSERT(i >= 0 && i < size()), _array[i + _offset];
    }
};
...
public:
    ...
    constexpr char operator[](int i) const
    {
        return AK_TOOLKIT_ASSERT(i >= 0 && i < size()), _lit[i + _offset];
    }
    AK_TOOLKIT_STRING_VIEW_OPERATIONS()
    constexpr ::std::size_t size() const
    {
...
constexpr string<N1_1 - 1 + N2, typename array_upgrade<TR>::type> operator+(const char (&l)[N1_1],
                                                                            string<N2, TR> const& r)
{
...
constexpr string<N1 + N2_1 - 1, typename array_upgrade<TL>::type> operator+(string<N1, TL> const& l,
                                                                            const char (&r)[N2_1])
{
